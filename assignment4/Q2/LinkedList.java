...
public class LinkedList<E> implements List<E> {
    ...
    public Iterator<E> iterator(int nextIndex) {
        if (nextIndex < 0 || nextIndex >= size) {
            throw new IndexOutOfBoundsException("Invalid nextIndex");
        }

        LinkedListIterator iterator = new LinkedListIterator();
        while (iterator.hasNext() && iterator.nextIndex() < nextIndex) {
            iterator.next();
        }
        return iterator;
    }
    ...
    public Iterator<E> iterator(Iterator<E> other) {
        ...
        int index = otherIterator.nextIndex();
        ...
        throw new NoSuchElementException("Iterators are not compatible");
    }
