...
public class ParkingLot {
	...
	public void park(int i, int j, Car c, int timestamp) {
		...
		// WRITE YOUR CODE HERE!	 Spot(car, timestamp)
		if (canParkAt(i,j,c))
        {
	...
	public int getTotalCapacity() {
		...
		return count; // REMOVE THIS STATEMENT AFTER IMPLEMENTING THIS METHOD
	}
	...
	private void populateDesignFromFile(String strFilename) throws Exception {
		...
		while (scanner.hasNext()) { 
			...
			for (int i=0; i<strings.length;i++){
				lotDesign[rowN][i]=Util.getCarTypeByLabel(strings[i].trim());
			}
			rowN++;
		}
		scanner.close();
