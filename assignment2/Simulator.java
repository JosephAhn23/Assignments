	...
	public void simulate() {
		...
		while (clock < steps) {  //2
			if (RandomGenerator.eventOccurred(probabilityOfArrivalPerSec)){
				Car generatedCar= RandomGenerator.generateRandomCar();
				incomingQueue.enqueue(new Spot(generatedCar,clock));
			}
		

			for (int i=0; i<lot.getNumRows(); i++) { //5
					...
					if (spot != null) {
						...
						if (duration == MAX_PARKING_DURATION) {
							...
							if (RandomGenerator.eventOccurred(departurePDF.pdf(duration))  ) {
								// Car leaves: remove the car from the lot
								lot.remove(i, j);
			...
			if (!incomingQueue.isEmpty()) {
				Car c=incomingQueue.dequeue().getCar();
				if (lot.attemptParking(c, clock)){
					for (int i=0; i<lot.getNumRows(); i++) { //5
						for (int j=0; j<lot.getNumSpotsPerRow(); j++) {
							lot.park(i,j,c,clock);
